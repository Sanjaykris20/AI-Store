...
class CartState(rx.State):
    ...
    def sync_cart_to_db(self):
        """Push the current cart items to Firebase."""
        from state.user_state import get_firebase_db, UserState
        try:
            # We fetch the async-free instance inside reflex state wrapper
            uid = self.get_state(UserState).firebase_uid
            db = get_firebase_db()
            if db and uid:
                db.child("users").child(uid).child("cart").set(self.cart_items)
        except Exception as e:
            print(f"Failed to sync cart: {e}")

    def load_cart_from_db(self, uid: str):
        """Load cart items from Firebase on login."""
        from state.user_state import get_firebase_db
        db = get_firebase_db()
        if db and uid:
            try:
                cart_data = db.child("users").child(uid).child("cart").get().val()
                if cart_data:
                    # In case of nested tuples from Pyrebase or empty gaps, sanitize to a clean list
                    self.cart_items = [item for item in cart_data if item is not None]
                    self.calculate_total()
                else:
                    self.cart_items = []
                    self.total_price = 0.0
            except Exception as e:
                print(f"Failed to load cart: {e}")
